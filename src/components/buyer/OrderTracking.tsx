      ...
      try {
        ...
        if (orderData) {
          const [productData, variantData] = await Promise.all([
            getProductById(orderData.product_id),
            orderData.variant_id ? getProductVariantById(orderData.variant_id) : Promise.resolve(null),
          ]);
        ...
        <div className="bg-white rounded-xl shadow-md p-6 mb-4">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-right">{product?.name || order.product_id}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Variant</span>
              <span className="font-semibold text-right">{variant?.name || order.variant_id || '-'}</span>
            </div>
            <div className="flex justify-between">
